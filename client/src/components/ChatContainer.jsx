    ...
    <div className='h-full overflow-hidden relative backdrop-blur-lg'>
      ...
      <div className='flex flex-col h-[calc(100%-64px)] overflow-y-scroll p-3 pb-6'>
        {messages.map((msg, index) => (
          <div
            key={index}
            className={`flex items-end gap-2 mb-3 ${msg.senderId === authUser._id ? 'justify-end' : 'justify-start'}`}
          >
            {msg.image ? (
              <img
                src={msg.image}
              ...
              <p
                className={`p-2 max-w-[200px] md:text-sm font-light rounded-lg break-all bg-violet-500/30 text-white ${msg.senderId === authUser._id
                  ? 'rounded-br-none'
                  : 'rounded-bl-none'
                  }`}
              >
